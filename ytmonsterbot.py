...
def ytmonster(task):
    ...
    try:
        if task == 'balance':
            ...
            req = requests.get('https://app.ytmonster.ru/api/?get-clients=get&token=' + token_work) #получение клиента
            json1 = json.loads(req.text)
            error = (json1["error"])
            print(error)
        elif task == 'close_client':
            req = requests.get('https://app.ytmonster.ru/api/?close-client=' + id + '&token=' + token_work) #закрытие клиента
            json1 = json.loads(req.text)
            error = (json1["error"])
            print(error)
        elif task == 'my_task':
            req = requests.get('https://app.ytmonster.ru/api/?my-tasks=ytview&offset=0&token=' + token_task) #получение заданий
...
print(req.text)
json1 = json.loads(req.text)
url = (json1["result"]["username"])
time.sleep(2)
print('\n'*100 + 'Бот запущен! Адрес: https://t.me/' + url)
@bot.message_handler(commands=["start"])
def test(message):
    buttons.buttons_start(bot, message)
    print('Сообщение отправлено!')


def keyboard_start(call):
    buttons.buttons_start(bot, call)
    print('Сообщение отправлено!')
...
def callback_worker(call):
    ...
    if call.data == "balance":
        ...
        if err != 'ok':
            buttons.back(bot, call, err)
        if req.text == """{"error":0,"response":[]}""":
            print('Нет рабочих клиентов')
            buttons.back(bot, call, 'Нет рабочих клиентов\nTelegram канал разработчика: https://t.me/GODIMONGO')
        else:
            json1 = json.loads(req.text)
            i = len(json1['response'])
            ...
            time.sleep(1)
            buttons.back(bot, call, client_info + '\nTelegram канал разработчика: https://t.me/GODIMONGO')
    elif call.data == "back":
        keyboard_start(call)
    elif call.data == 'change':
        buttons.back(bot, call, 'Для смены токена от ytmonster Есть 2 команды: /newtokenytmonster_work (токен для выполнения заданий без скобок)- меняет токен для выполнения заданий\n /newtokenytmonster_task (токен для добавления заданий без скобок )- меняет токен для выполнения заданий')
    elif call.data == 'clear_log':
        with open('log.txt', 'w') as f:
            f.write('\nclear log: ' + str(datetime.now()))
            f.close()
            buttons.back(bot, call, 'Лог очищен')
    elif call.data == 'close_client':
        buttons.back(bot, call, 'Для закрытия клиента используйте команду: /close_client айди клиента \n Пример: /close_client fea9f8')
    elif call.data == 'my_task':
        req, err = ytmonster('my_task')
        if err != 'ok':
            buttons.back(bot, call, err)
        else:
            my_task('yt_vew', req, call)
        print('скоро')
        ...
        bot.send_message(call.message.chat.id, 'Производится замер скорости интеренета! Результат появится в течении 1-3 минут! Напомню! Некоторые ОС не поддерживают данную функцию!')
        time.sleep(1)
